...
const ChooseVolunteerButton = () => {
    ...
    <div className='card-button'>
        <div>
            <button ref={buttonRef} type='button' className='choose-volunteer-button' onClick={() => setIsPopupOpen(prevIsPopupOpen => !prevIsPopupOpen)}>
                <span>CHOOSE VOLUNTEER</span>
                <FontAwesomeIcon style={{margin: '0 5px'}} icon={faCaretDown} />
            </button>
            {
                isPopupOpen && (
                    <ChooseVolunteerPopup popupPositionY={popupPositionY} popupPositionX={popupPositionX} setIsPopupOpen={setIsPopupOpen}/>
                )
            }
        </div>
    </div>
